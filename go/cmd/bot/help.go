...
func sendHelp(ctx context.Context, b *tgbot.Bot, chatID int64, cfg *config.Config, mode string, dbConnected bool, logger *slog.Logger) {
	...
	if _, err := b.SendMessage(ctx, &tgbot.SendMessageParams{
		...
		logger.Error("не удалось отправить /help", "error", err, "chat_id", chatID)
	}
}
